...
class FileService
{
    public function guardarArchivosAjax($archivos, $model, $folder, $docType = null, $docSubType = null)
    {
        ...
        if (count($archivos) > 0) {
            foreach ($archivos as $a) {
                ...
                if ($upload) {
                    $id = $model->id;
                    if (empty($model->id) && isset($model->code)) {
                        $id = $model->code;
                    }
                    $f = File::create([
    ...
    public function guardarArchivos($archivos, $data, $private = false)
    {
        // dd(count($archivos));
        $files_saved = [];
        $record_id = null;
