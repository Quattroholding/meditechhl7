...
<div class="flex items-center justify-between">
    <div class="flex items-center space-x-2">
        ...
        <div wire:loading
             @if(isset($function_param))
                 wire:target="{{$function}}({{$function_param}})"
             @else
                 wire:target="{{$function}}"
             @endif
             class="flex items-center text-blue-600 px-3 py-1">
        ...
        <!-- Mensaje de guardado -->
        @if($saved)
            <div class="flex items-center text-green-600 px-3 py-1" id="saved-message{{rand(3,4)}}">
